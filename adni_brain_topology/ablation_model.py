...
class GAT(nn.Module):
    def __init__(self, model_config):
        ...
        self.gat_layers.append(GATConv(
            ...
            self.activation, bias=False, allow_zero_in_degree=True))
        # hidden layers
        for l in range(1, self.num_layers):
            ...
            self.gat_layers.append(GATConv(
                ...
                model_config['residual'], self.activation, bias=False, allow_zero_in_degree=True))
        self.classification = nn.Linear(self.node_each_graph * model_config['hid_dim'], model_config['n_classes'])
...
class VGAE(nn.Module):
    def __init__(self, model_config):
        ...
        self.node_each_graph = model_config['node_each_graph']
        layers = [GraphConv(self.in_dim, self.hid_dim, activation=F.relu, norm='both'),
                  GraphConv(self.hid_dim, self.hid_dim, activation=lambda x: x, norm='both'),
                  GraphConv(self.hid_dim, self.hid_dim, activation=lambda x: x, norm='both')]
    ...
    def encoder(self, g, features):
        ...
        sampled_z = mean + gaussian_noise * torch.sigmoid(log_std)
        return sampled_z
...
# VGAE_v2
class VGAE_V2(nn.Module):
    def __init__(self, model_config):
        super(VGAE_V2, self).__init__()
        self.in_dim = model_config['in_dim']
        self.hidden_dim = model_config['hid_dim']
        self.n_layers = model_config['n_layers']
        self.node_each_graph = model_config['node_each_graph']
        self.activation = model_config['activation']
        layers = []
        layers.append(GraphConv(self.in_dim, self.hidden_dim, activation=self.activation, allow_zero_in_degree=True))
        for i in range(self.n_layers - 1):
            layers.append(GraphConv(self.hidden_dim, self.hidden_dim, activation=self.activation, allow_zero_in_degree=True))
        self.layers = nn.ModuleList(layers)
        self.mask_ratio = model_config['mask_ratio']
        self.batch = nn.BatchNorm1d(self.in_dim)
        self.classification = nn.Linear(self.node_each_graph * model_config['hid_dim'], model_config['n_classes'])

    def masking(self, feat):
        n = feat.shape[0]
        mask = torch.FloatTensor(np.ones(n) * self.mask_ratio)
        masks = torch.bernoulli(1. - mask).unsqueeze(1)
        feat = masks * feat
        return feat
    ...
    def forward(self, graph, batch_size):
        ...
        logits = self.classification(feat)
        return logits
    ...
    def encode_decode(self, graph):
        ...
        for i, layer in enumerate(self.layers):
            feat = layer(graph, feat)
        feat = self.masking(feat)
        reconstruct_feat = torch.matmul(feat, feat.t())
        return reconstruct_feat
